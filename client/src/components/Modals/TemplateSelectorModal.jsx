            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">Select a design that best fits your personal brand.</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                {/* Grid */}
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/50 max-h-[60vh] overflow-y-auto">
                    {templates.map((template) => {
                        ...
                        return (
                            <button
                                ...
                                className={`group relative flex items-start text-left p-6 rounded-xl border-2 transition-all duration-300 hover:shadow-lg ${isSelected
                                        ? 'border-blue-600 bg-blue-50/10 ring-1 ring-blue-600'
                                        : 'border-gray-200 bg-white hover:border-gray-300'
                                    }`}
                            >
                                <div className={`p-4 rounded-lg mr-5 ${template.color} ${template.borderColor} border`}>
                                    <Icon className="w-8 h-8" />
                                </div>

                                <div>
                                    <h3 className={`font-bold text-lg mb-1 ${isSelected ? 'text-blue-700' : 'text-gray-900 group-hover:text-blue-600 transition-colors'}`}>
                                        {template.name}
                                    </h3>
